...
def generar_factura_pdf(factura, pedido, cliente, detalles) -> str:
    """Genera el PDF de la factura y retorna la ruta absoluta del archivo."""
    if not LOGO_PATH.exists():
        raise HTTPException(status_code=500, detail="Logo de NEODOMUS no encontrado")

    s = _styles()
    numero_factura = factura.numero_factura
    archivo = FACTURAS_DIR / f"factura_{numero_factura.replace('/', '-')}.pdf"
    ...
    historia.append(Paragraph(
        f"Generado el {datetime.now().strftime('%d/%m/%Y %H:%M')} - NEODOMUS · "
...
async def enviar_factura_por_correo(to_email: str, pdf_path: str) -> bool:
    ...
    return await send_email_with_attachment(to_email, subject, body, pdf_path)


# ── Factura para Citas (servicios standalone) ────────────────────────────────


def generar_factura_cita_pdf(factura, cita, cliente) -> str:
    """Genera el PDF de la factura para una cita y retorna la ruta del archivo."""
    if not LOGO_PATH.exists():
        raise HTTPException(status_code=500, detail="Logo de NEODOMUS no encontrado")
...
def crear_factura_cita(db, cita, cliente) -> None:
    """Crea la factura de una cita, genera su PDF y envía el correo.

    El envío de correo es fire-and-forget (no bloquea la respuesta).
    Si el correo falla, la factura queda registrada con enviada_por_correo=False.
    """
    import asyncio
    ...
    numero_factura = f"FAC-{datetime.now().strftime('%Y%m%d')}-{cita.id_cita:06d}"
    factura = Factura(
    ...
    try:
        pdf_path = generar_factura_cita_pdf(factura, cita, cliente)
    ...
    async def _enviar():
        await send_email_with_attachment(cliente.email, subject, body, pdf_path)
