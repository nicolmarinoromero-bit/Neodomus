...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [passwordResetRequired, setPasswordResetRequiredState] = useState(
    () => localStorage.getItem(PASSWORD_RESET_KEY) === '1',
  );

  const setPasswordResetRequired = (flag: boolean) => {
    setPasswordResetRequiredState(flag);
    if (flag) {
      localStorage.setItem(PASSWORD_RESET_KEY, '1');
    } else {
  ...
  const googleLogin = async (credential: string) => {
    try {
      ...
      const data = response.data;
      const userRol = data.rol || data.role || 'cliente';
      localStorage.setItem('access_token', data.access_token);
      localStorage.setItem('refresh_token', data.refresh_token);
  ...
  return (
    <AuthContext.Provider value={{ user, rol, isAuthenticated, loading, passwordResetRequired, setPasswordResetRequired, login, googleLogin, logout, refreshUserProfile }}>
      {children}
