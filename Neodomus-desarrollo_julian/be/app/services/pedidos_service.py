...
def _asignar_tecnico(db: Session, tipo_servicio: str, fecha: date) -> Tecnico | None:
    """Elige el primer técnico activo con especialidad compatible que esté
    libre ese día. Devuelve None si no hay."""
    ...
    for t in candidatos:
        if not compatible_especialidad(tipo_servicio, t.certificacion_t):
            continue
        if not tecnico_ocupado(db, t.id_tecnico, fecha):
            return t
    return None
...
def _crear_orden_instalacion(
    ...
    if id_tecnico_sel is not None:
        ...
        if tecnico_ocupado(db, tecnico.id_tecnico, fecha):
            raise HTTPException(
                status_code=400,
                detail="El técnico seleccionado ya está ocupado ese día. Elige otro técnico o fecha.",
            )
    else:
        tecnico = _asignar_tecnico(db, tipo_servicio, fecha)
    ...
    cita = Cita(
        ...
        estado="Confirmada" if tecnico else "Pendiente",
    )
    db.add(cita)
    db.commit()
    db.refresh(cita)
    ...
    if tecnico and tecnico.usuario and tecnico.usuario.email:
...
async def _crear_factura_y_enviar(
    ...
    """Crea la factura, genera su PDF y la envía por correo.

    Si el correo falla, el pago NO se marca rechazado: solo se registra el
    error y la factura queda lista para ser reenviada.
    """
    numero_factura = f"FAC-{datetime.now().strftime('%Y%m%d')}-{pedido.id_pedido:06d}"
    factura = Factura(
    ...
    try:
        pdf_path = generar_factura_pdf(factura, pedido, cliente, detalles)
    ...
    if factura.pdf_path:
        try:
            enviado = await enviar_factura_por_correo(cliente.email, factura.pdf_path)
        except Exception as e:
            enviado = False
...
def _asignar_entrega(db: Session, pedido: Pedido, cliente: Cliente) -> dict | None:
    ...
    for delta in range(1, 6):
        ...
        if not _dia_es_laboral(f):
            continue
        libre = next(
            (t for t in candidatos if not tecnico_ocupado(db, t.id_tecnico, f)),
            None,
        )
